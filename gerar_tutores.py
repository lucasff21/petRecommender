...
for user_id in range(1, NUM_TUTORES + 1):
    tutores_data.append({
        ...
        'pref_idade': random.choice(['Filhote', 'Adulto', 'Idoso', 'Indiferente']),
        'pref_porte': random.choice(['Pequeno', 'Médio', 'Grande', 'Indiferente']),
        'pref_sexo': random.choice(['Macho', 'Fêmea', 'Indiferente']),
        'nivel_energia_desejado': random.randint(1, 5),
        'necessidade_guarda_desejada': random.randint(1, 5),
        'tolerancia_pelo': random.randint(1, 5),
        'tolerancia_latido': random.randint(1, 5)
    })
